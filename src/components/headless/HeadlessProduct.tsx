...
export const useProductLogic = () => {
  ...
  const handleBuyNow = async () => {
    ...
    // Build cart items manually to avoid race condition with React state
    setIsBuyingNow(true)
    try {
      const buyNowItems: import('@/contexts/CartContext').CartItem[] = [{
        key: `${product.id}:${variantToAdd?.id || ''}:${selectedPlan?.id || ''}`,
      ...
      // Save to useCheckoutState (localStorage) - required for checkout page
      saveCheckoutState({
      ...
      try {
        sessionStorage.setItem('checkout_cart', JSON.stringify({ items: buyNowItems, total: currentP * quantity }))
      ...
      console.error('Error in handleBuyNow checkout:', error)
      toast({
